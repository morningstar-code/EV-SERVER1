...
RegisterCommand('+useKeyFob', function()
    if Throttled("ev-doors:doorKeyFob", 1000) then return end
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (not IsPedInAnyVehicle(player)) then
        ...
        if (itemid == "Boombox") then
         ...
         if (itemid == "problemlaptop") then
            TriggerEvent('ev-boosting:open:boostingApp')
        end 
--[[         if (itemid == "racingtablet") then -- rahe racing tablet #mikey
            print('ffs')
            TriggerEvent('rahe-racing:client:open:tablet')
            print('dick')
        end ]]
        if (itemid == "Box") then
    ...
    if (itemid == "pdbadge") then
        RPC.execute("ev-gov:police:showBadge", json.decode(ItemInfo.information))
    end
    ...
    if (itemid == "matrixredpill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["ev-taskbar"]:taskBar(3000,"Taking the Red Pill",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",24,1200)
    ...
    if (itemid == "weedoz") then
      local finished = exports["ev-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if itemid == "weedbong" then
        ...
        if currentHits == 0 then
            ...
            local finished = exports["ev-taskbar"]:taskBar(12000, "Hitting Bong", false, false, playerVeh)
    ...
    if (itemid == "advlockpick") then
        ...
        if myJob ~= "news" then
            if not exports["ev-boosting"]:isInBoost() == true then
    ...
    if (itemid == "trackerdisabler")  then
        TriggerEvent("ev-boosting:antiPolice") 
    end  
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["ev-taskbar"]:taskBar(5000,"Reloading flamethrower",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        local finished = exports["ev-taskbar"]:taskBar(5000,"Armor",true,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
    ...
    if (itemid == "gunpaintbucket") then
        if not (hasEnoughOfItem("refinedsteels", 3, false) and hasEnoughOfItem("refinedplastics", 3, false)) then
            ...
            local finished = exports["ev-taskbar"]:taskBar(5000, "Filling Paint Gun", true, false)
    ...
    if (itemid == "Peecup_empty") then
        ...
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            local cid = exports["isPed"]:isPed("cid")
            TriggerServerEvent("ev-drugeffects:useEmptyPeeCup", cid)
        end
        else
    ...
    if (itemid == "npolaroid_camera") then
    ...
    if (itemid == "nitrous") then
        ...
        local isElectric = exports['ev-vehicles']:IsVehicleElectric(currentVehicle)
            ...
            if status == "Empty" then
                ...
                elseif not IsToggleModOn(currentVehicle, 18) then
                    ...
                    if (finished == 100 and not cancelNos) then
                        ...
                        if TriggerEvent("vehicle:addNos", "large") then 
    ...
    if (itemid == "test_bottle") then
        TriggerEvent( "player:receiveItem","test_pill",1)
        TriggerEvent("inventory:DegenLastUsedItem",3)
    end
    ...
    if (itemid == "photo") then
        local ItemInfo = GetItemInfo(slot)

        local image = json.decode(ItemInfo.information).image
        local location = json.decode(ItemInfo.information).location

        TriggerEvent("ps-camera:usePhoto", image, location) 
    end
    ...
    if (itemid == "weightlosspill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["ev-taskbar"]:taskBar(3000,"Losing weight! 💊",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            remove = true
            TriggerEvent("animation:PlayAnimation","cancel")
            Citizen.Wait(100)
    ...
    if (itemid == "Desomorphine") then
        ...
        if success then
            local cid = exports["isPed"]:isPed("cid")
            TriggerServerEvent("ev-drugeffects:drugsTaken", cid, "desomorphine")
    ...
    if (itemid == "steroid_syringe") then
        loadAnimDict('clear@custom_anim');
        TaskItem("clear@custom_anim", "adrenaline_clip", 49,3000,"Injecting steroids.","hadsteroids", true, itemid, playerVeh)
    end

    if (itemid == "adderall_pill") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["ev-taskbar"]:taskBar(3000,"Gotta focus!",false,false,playerVeh)
        if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
            local drugEffect = true
    ...
    if itemid == "C4_dev" then 
        TriggerEvent('ev-bombs:itemUsed')
    end
    ...
    if (itemid == "cokeline") then
        ...
        if itemInfo and itemInfo.quality > 0 then
            ...
            if currentUses == 0 then
                ...
                local finished = exports["ev-ui"]:taskBarSkill(3000, math.random(15, 20))
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    local cid = exports["isPed"]:isPed("cid")
                    TriggerServerEvent("ev-drugeffects:drugsTaken", cid, "cocaine")
                end
            end
        end
    end
    ...
    if (itemid == "crackpipe") then
        local itemInfo = GetItemInfo(slot)
        if itemInfo and itemInfo.quality > 0 then
            if hasEnoughOfItem("1gcrack", 1, false) then
                local finished = exports["ev-ui"]:taskBarSkill(2400, math.random(12, 18))
                if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) and hasEnoughOfItem("1gcrack", 1, false) ) then
                    ...
                    TriggerEvent("removeCrack") 
                    local cid = exports["isPed"]:isPed("cid")
                    TriggerEvent("fx:run", "crack", 180, -1, (itemid == "badlsdtab" and true or false)) 
                    TriggerServerEvent("ev-drugeffects:drugsTaken", cid, "crack") 
                end
            else
                TriggerEvent("DoShortHudText", "You ran out of crack! :(", 2) 
            end
        end    
    end
            ...
            if #currentPurities == 0 then
                ...
                if (finished == 100 and hasEnoughOfItem(itemid, 1, false)) then
                    ...
                    if success then
                        ...
                        local cid = exports["isPed"]:isPed("cid")
                        TriggerServerEvent("ev-drugeffects:drugsTaken", cid, "meth")
                    end
    ...
    if sandwichItems[itemid] or deanworldFood[itemid] then
        ...
        remove = customMarketItems[itemid]
        TriggerEvent("inv:wellfedNoStress")
    end
    ...
    if itemid == "boggsproteinbar" then
        ...
        if success then
            TriggerEvent("ev-buffs:applyBuff", "boggs", { { buff = "bikeStats", percent = 1.0, timeOverride = 60 * 1000 } })
    ...
    if (itemid == "harness") then
        ...
        finished = exports["ev-taskbar"]:taskBar(20000,"Harness")
    ...
    if itemid == "npolaroid_camera" then
        TriggerEvent("ev-polaroid-capture:camera:start")
    end
...
AddEventHandler('randPickupAnim', function()
    ...
    local _, isAnimalModel = pcall(function () return exports["ev-misc-code"]:isAnimalModel(GetEntityModel(ped)) end)
...
AddEventHandler('ev-business:examineItem', function()
    ...
    if type(failed) == 'string' then
        TriggerEvent('chatMessage', "", 1, " " .. failed);
        return
    end

    TriggerEvent('chatMessage', "", 1, " ^1Item current quality is: " .. Quality .. " we will repair a total of %" .. PercentRepair);

    for k, v in pairs(itemListInfo[1]) do
        local itemid = v.itemid
        local cost = math.ceil( v.amount * PercentRepair/100 )
        if Quality == 0 then cost = cost * 2 end
        TriggerEvent('chatMessage', "", 1, itemid .. " ^1required: " .. cost);
    end
end)
...
function hasEnoughOfItem(itemid, amount, shouldReturnText, tonumbercheckQuality, metaInformation)
...
AddEventHandler('inv:lockPick', function(isForced, inventoryName, slot, itemType) 
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 and (IsPedAPlayer(driverPed) or not IsPedDeadOrDying(driverPed)) then
            ...
            exports['ev-vehicles']:LockpickVehicleDoor(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
            ...
            Citizen.SetTimeout(math.random(120, 7200) * 1000, function ()
                TriggerServerEvent('ev:vehicles:hotPlate', plate, '10-60')
            end)

            if result.success and not isForced then
                exports['ev-flags']:SetVehicleFlag(targetVehicle, 'isStolenVehicle', true)
            end

            lockpicking = false

            local batman = math.random(1,20)
            if batman <= 5 then
                ...
                --exports["ev-dispatch"]:dispatchadd('10-60', "Attempted Vehicle Robbery")
            end
    else
        if targetVehicle ~= 0 then

            if exports["ev-vehicles"]:HasVehicleKey(targetVehicle) then lockpicking = false return end
            ...
            exports['ev-vehicles']:HotwireVehicle(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
...
RegisterUICallback("ev-ui:inv:moneycaseAction", function(data, cb)
...
AddEventHandler("crackpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["ev-taskbar"]:taskBar(20000, "Packing crack pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("cokeline:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["ev-taskbar"]:taskBar(15000, "Filling with " .. cConfig.fillString, false, false)
    if finished ~= 100 then return end
...
AddEventHandler("methpipe:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["ev-taskbar"]:taskBar(10000, "Packing meth pipe", false, false)
    if finished ~= 100 then return end
...
AddEventHandler("weedbong:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["ev-taskbar"]:taskBar(20000, "Packing Bong", false, false)
    if finished ~= 100 then return end

    if not hasEnoughOfItem(pOriginItemId, itemInfo.amount, false) then return end

    local maxHits = 5
    if currentHits + itemInfo.amount > maxHits then
    ...
    currentHits = currentHits + itemInfo.amount
...
RegisterUICallback('ev-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
...
function applyPaint(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo, minRng, maxRng)
    ...
    local finished = exports["ev-taskbar"]:taskBar(15000, "Painting gun", false, false)
    if finished ~= 100 then return end
