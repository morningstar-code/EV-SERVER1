...
$(function () {
  ...
  $('.apply-tattoo').on('click', function () {
    let tattooZone = $(this).parent().parent().parent().attr('id');
    let tattooID = $(this).parent().parent().find('.input-number').val();
    if ((tattooZone || tattooID) == undefined) {
      console.log(tattooZone, tattooID);
      return;
    }
    $.post(
      'https://raid_clothes/addTattoo',
      JSON.stringify({
        tattooZone: tattooZone,
        tattooID: tattooID,
      }),
    );
  });

  $('.remove-tattoo').on('click', function () {
    let tattooZone = $(this).parent().parent().parent().attr('id');
    let tattooID = $(this).parent().parent().find('.input-number').val();
    if ((tattooZone || tattooID) == undefined) {
      console.log(tattooZone, tattooID);
    ...
    $.post(
      'https://raid_clothes/removeTattoo',
      JSON.stringify({
        tattooZone: tattooZone,
      }),
    );
    ...
    if (currentMenu.is($('#clothing_shop')) || $(ele).parents('.panel').hasClass('hair')) {
      ...
      if ($('#skin_female').val() == 1 || $('#skin_male').val() == 1) {
        try {
          ...
          let isNotValid = true;
          let gender = 'male';
          if ($('#skin_female').val() >= 1 && $('#skin_male').val() == 0) gender = 'female';
      ...
      if ($(ele).parents('.panel').attr('id') == 'skins') {
        ...
        let nameId = '';
        if (currentMenu.is($('#barber_shop'))) nameId = 'hair';
        else nameId = $(ele).parent().parent().parent().attr('id').split('#')[0];
        $.post(
          'https://raid_clothes/updateclothes',
          JSON.stringify({
            name: nameId,
      ...
      $.post('https://raid_clothes/previewtats', JSON.stringify({ tats }));
    }
  }
  ...
  function SaveHeadOverlay(ele) {
    ...
    $.post(
      'https://raid_clothes/saveheadoverlay',
      JSON.stringify({
        name: id,
        value: inputs.eq(0).val(),
        opacity: opacity,
  ...
  function AddPalettes() {
    ...
    $('.color_palette').on('click', function () {
      ...
      if ($(this).parents('.panel').hasClass('hair')) {
        $.post(
          'https://raid_clothes/savehaircolor',
          JSON.stringify({
        ...
        $.post(
          'https://raid_clothes/saveheadoverlaycolor',
          JSON.stringify({
            firstColour: palettes.eq(0).find('.active').attr('value'),
            secondColour: palettes.eq(1).find('.active').attr('value'),
