...
function CreateShowroomVehicle(showroom, vehicleName)
    ...
    if preset then
        exports["ev-vehicles"]:SetVehicleMods(vehicle, preset.mods and preset.mods or {})
        exports["ev-vehicles"]:SetVehicleAppearance(vehicle,  preset.appearance and preset.appearance or {})
        if preset.fitment and preset.fitment.w_width then
            DecorSetBool(vehicle, "ev-wheelfitment_applied", true)
            DecorSetFloat(vehicle, "ev-wheelfitment_w_width", preset.fitment.w_width)
            DecorSetFloat(vehicle, "ev-wheelfitment_w_fl", preset.fitment.w_fl)
            DecorSetFloat(vehicle, "ev-wheelfitment_w_fr", preset.fitment.w_fr)
            DecorSetFloat(vehicle, "ev-wheelfitment_w_rl", preset.fitment.w_rl)
            DecorSetFloat(vehicle, "ev-wheelfitment_w_rr", preset.fitment.w_rr)
        end
    end
    return vehicle
end
...
function openShowroom(showroom, worker)
    ...
    local isWorker = exports["ev-business"]:IsEmployedAt(showroom)
    shopWorker = isWorker
...
function getVehiclesTable(shop)
    local _vehicles = RPC.execute("ev-carshop:getVehicles", shop)
...
RegisterNUICallback("Buy", function(data, cb)
    CloseNui()
    local result, data = RPC.execute("ev-carshop:change", currentLocation, currentVehicle, data.modelcar)
