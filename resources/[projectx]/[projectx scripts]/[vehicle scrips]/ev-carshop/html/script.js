...
window.addEventListener('message', function(event) {
    ...
    if (event.data.type === 'display') {
        ...
        for (var [key, value] of Object.entries(data.categorys)) {
            $('.top-menu').append(`<a href="#${value}" onclick="menuVehicle(event)" value="${value}">${value}</a>`);
        }
    ...
    if (event.data.type === 'updateVehicleInfos') {
        ...
        for (i = 0; i < (dataVehicles.length); i++) {
            if (dataVehicles[i].qtd > 0) {
                if (vehicleSelected.modelcar === dataVehicles[i].model) {
                    currModel = dataVehicles[i].model;
                }
            }
        }

        if (vehicleSelected.modelcar === currModel) {
            if (showBuy) {
                $('#actionButton').append(`<a class="buyButton" style=" color:#FC1259;" onclick="buyVehicle()"> Put On Show </a>`);
            }
        }
    }
});
