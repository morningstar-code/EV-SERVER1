RPC.register("ev-gov:police:showBadge", function(pSource, pInfo, pCoords)
    ...
    for _, serverID in ipairs(players) do
        TriggerClientEvent("ev-gov:police:showBadge", serverID, pSource, info)
    end
end)

RPC.register("ev-gov:police:getBadge", function(pSource)
    local src = pSource
