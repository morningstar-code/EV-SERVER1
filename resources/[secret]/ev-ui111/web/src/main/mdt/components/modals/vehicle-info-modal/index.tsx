...
class VehicleInfoModal extends React.Component<any> {
    ...
    saveImpoundData = async (data: { id: number, strike: number, locked_until: number, release_paid: number, released: number, release_date: number }) => {
        ...
        await nuiAction('updateImpoundData', data, {});
    ...
    async componentDidMount() {
        const impounds = await nuiAction('getVehicleImpounds', { vin: this.props.vehicle.vin }, { returnData: [
            {
                id: '1',
                report_id: '1',
        ...
        const history = await nuiAction('getVehicleOwnershipHistory', { vin: this.props.vehicle.vin }, { returnData: [
            {
