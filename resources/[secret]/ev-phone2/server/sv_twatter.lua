...
function hasTwatterAdmin(id)
    local numericId = tonumber(id)
    
    if numericId == 1003 or numericId == 1006 then
        return true 
    else 
        return false
    end
...
function getTwatterEntries()
    -- Query to fetch twat entries
    local query = [[
        SELECT
            t.text,
            t.timestamp,
            c.id AS character_id,  -- Include character ID in the results
            c.first_name,
            c.last_name,
            ptb.is_blue
        FROM
    ...
    -- Fetch Twatter admin IDs
   -- local adminQuery = [[SELECT cid FROM _twatter_admins]]
    -- local adminResult = Await(SQL.execute(adminQuery))

    -- local adminIds = {}
    -- for _, admin in pairs(adminResult) do
    --     adminIds[admin.cid] = true  -- Use a table for fast lookup
    -- end

    local entries = {}

    for _, v in pairs(result) do
        -- Check if the character ID is in the admin IDs table
        -- local hasTwatterAdmin = adminIds[v.character_id] ~= nil
        ...
        entries[#entries + 1] = {
            ...
            -- isGold = hasTwatterAdmin,  -- Set isGold based on admin check
        }
    end
...
function addTwatterEntry(pCharacterId, pFirstName, pLastName, pText)
    ...
    -- local hasGold = hasTwatterAdmin(pCharacterId)
    -- print("Gold?",hasTwatterAdmin)
