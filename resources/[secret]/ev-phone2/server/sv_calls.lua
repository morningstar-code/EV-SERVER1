...
function establishPhoneCall(callId, pType, pName)
    ...
    if call then
        ...
        TriggerEvent("v:voice:phone:server:call:start", tonumber(call.caller.id), tonumber(call.target.id), callId)
...
function completePhoneCall(callId, pType, pName)
    ...
    if call then 
        ...
        TriggerEvent("v:voice:phone:server:call:end", tonumber(call.caller.id), tonumber(call.target.id), callId)
