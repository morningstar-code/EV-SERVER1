...
-- for some people who dosnt know how to fucking read code... when someone leaves the server it deletes theyre advert
AddEventHandler("playerDropped", function (reason)
    local src = source
    local user = exports["ev-lib"]:getUserData(src)

    local data = Await(SQL.execute("SELECT * FROM _phone_advert WHERE cid = @cid", {
        cid = user.character.id
    }))

    -- check if a person even has an ad and if it does it deletes it
    print("person dropped checking for adverts...")
    if not data or #data == 0 then
        Citizen.Wait(1000)

        print("cid: " .. user.character.id .. " has no adverts")
        return
    else 
        Await(SQL.execute("DELETE FROM _phone_advert WHERE cid = @cid", {
            cid = user.character.id
        }))

        Citizen.Wait(1000)
        print("cid: " .. user.character.id .. " has a advert deleting...")
    end
end)