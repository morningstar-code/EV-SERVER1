...
RegisterUICallback("ev-ui:activateSelfieMode", function(data, cb)
...
RegisterUICallback("ev-ui:phone:hasNotificationChanged", function(data, cb)
...
AddEventHandler('ev-inventory:itemCheck', function(itemId, hasItem)
  ...
  for _,item in ipairs(phoneItems) do 
    hasPhone = exports['ev-inventory']:hasEnoughOfItem(item, 1, false, true) or hasPhone
  end
  ...
  exports["ev-ui"]:sendAppEvent("phone", { action = "phone-state-update", hasPhone = hasPhone })
end)
