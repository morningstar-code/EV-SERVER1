...
AddEventHandler("phone:call:receive", function(pNumber, pCallId)
  SendUIMessage({
    ...
    data = {
      ...
      number = pNumber, -- pIsPayPhone and "UNKNOWN NUMBER" or pNumber
      callId = pCallId
    }
  })
  isRinging = true
  ...
  while isRinging and not notifcations do
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 2.0, "ringing", 0.5)
    Wait(3000)
  end
end)
...
AddEventHandler("phone:call:dialing", function(pNumber, pCallId)
  ...
  while isDialing do
    TriggerEvent("InteractSound_CL:PlayOnOne", "dialing", 0.5)
    Wait(3200)
  end
end)
...
AddEventHandler("phone:call:inactive", function(pNumber, pCallId)
...
-- dial from phone
RegisterUICallback("ev-ui:callStart", function(data, cb)
...
AddEventHandler("phone:call911", function(args, pMakeCall, pAnon)
  ...
  if RPC.execute("police:is911Banned", cid) then -- in police script
  ...
  if pMakeCall then
    availableNumber = RPC.execute("ev-dispatch:get911Number") -- in dispatch script
...
-- answer from phone
RegisterUICallback("ev-ui:callAccept", function(data, cb)
...
-- end from phone
RegisterUICallback("ev-ui:callEnd", function(data, cb)
  local call_id = data.meta.callId
  local success, message = RPC.execute('phone:callEnd', call_id)
  cb({ data = {}, meta = { ok = success, message = message }})
end)

RegisterUICallback("ev-ui:togglePhoneNotificationSounds", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  local status = data.status
  notifcations = status == "on" and true or false
  --RPC.execute("ev-phone:silenceCallsForServerId") -- TODO;
end)
...
function HasValidPhoneItem()
  ...
  for _, item in ipairs(phoneItems) do
    if exports["ev-inventory"]:hasEnoughOfItem(item, 1, false, true) then
      return true  
    end
  end
...
AddEventHandler("ev-ui:application-closed", function (name, data)
