...
RegisterUICallback("ev-ui:getCars", function(data, cb)
  local data = RPC.execute("v:vehicles:getPlayerVehiclesWithCoordinates", data.character.id)
...
RegisterUICallback("ev-ui:carActionSpawn", function(data, cb)
  ...
  local success = NPX.Procedures.execute('v:vehicles:respawnVehicle', vehData.vin, {
    x = coords.x,
    y = coords.y,
    z = coords.z
  }, coords.h, false)
...
RegisterUICallback("ev-ui:carActionSell", function(data, cb)
  ...
  if not vehicleCoords then return end
  TriggerEvent('v:vehicles:sellPhone', data.car.vin, data.stateId, data.price + 0.0)
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
