...
AddEventHandler("phone:ping:receiveItemCheck", function(pCoords, pSrc, pCharacterName, pPingId, pItemCheck)
  local hasItem = exports["ev-inventory"]:hasEnoughOfItem(pItemCheck, 1, false, true)
  if not hasItem then return end

  TriggerEvent("phone:ping:receive", pCoords, pSrc, pCharacterName, pPingId)
end)

RegisterUICallback("ev-ui:pingSend", function(data, cb)
...
RegisterUICallback("ev-ui:pingAccept", function(data, cb)
...
RegisterUICallback("ev-ui:pingReject", function(data, cb)
  local pingId = data.pingId
  if not pingId then return end
