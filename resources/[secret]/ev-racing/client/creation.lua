...
local function updateObjects()
    if object1 == nil then return end
...
function startRaceCreation(options)
    ...
    if raceName == nil then
        raceName = KBInput("Race Name", "", 30)
    end

    while raceName ~= nil and options.raceName == nil and RPC.execute("ev-racing:isRaceNameTaken", raceName) == true do
    ...
    if raceCategory == nil then
        raceCategory = KBInput("Race Category", "", 30)
    end
...
function finishRaceCreation()
    ...
    TriggerServerEvent("ev-racing:recieveCreateData", raceName, raceType, raceCategory, raceThumbnail, raceMinLaps, checkpoints)
...
RegisterNetEvent("ev-racing:cmd:racecreate")
AddEventHandler("ev-racing:cmd:racecreate", function(options)
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
        startRaceCreation(options)
    end
end)

RegisterNetEvent("ev-racing:cmd:racecreatedone")
AddEventHandler("ev-racing:cmd:racecreatedone", function()
    finishRaceCreation()
end)

RegisterNetEvent("ev-racing:cmd:racecreatecancel")
AddEventHandler("ev-racing:cmd:racecreatecancel", function()
    cancelRaceCreation()
end)
