...
function initRace(race)
  ...
  additionalTotalTime = 0
  additionalLapTime = 0
  TriggerEvent("ev-racing:api:currentRace", curRace)
...
function startCheckpointsHandler()
  Citizen.CreateThread(function()
    ...
    TriggerServerEvent("ev-racing:updatePositionInfo", curRace.eventId, totalDist, straightDistToCheckpoint, curCheckpointIndex)
      ...
      if hasHitCheckpoint(curRace, curCheckpoint.pos, checkpointRadius, curCheckpoint.hdg, lastPlyPos, plyPos) then
        ...
        if curCheckpointIndex > numOfCheckpoints then -- If finish checkpoint is hit
          ...
          if time < bestLapTime then
            print("New best lap time: " .. time)
            bestLapTime = time
          end
          SendNUIMessage({HUD={lap=math.min(curLap, curRace.laps), bestLapTime=bestLapTime}})
          if curLap > curRace.laps then -- race finished
            print("finished with time: " .. time)
            print("bestLapTime: " .. bestLapTime)
            print("additionalTotalTime: " .. additionalTotalTime)
            -- need to get the total time race has been going on for using the raceStartTime
            additionalTotalTime = additionalTotalTime + GetGameTimer() - raceStartTime
            RPC.execute("ev-racing:finishedRace", curRace.eventId, additionalTotalTime, bestLapTime)
            break
          end
        ...
        if curRace.showPosition or (curRace.showPosition == nil and config.nui.hud.showPosition) then TriggerServerEvent("ev-racing:updatePositionInfo", curRace.eventId, totalDist, straightDistToCheckpoint, curCheckpointIndex) end
