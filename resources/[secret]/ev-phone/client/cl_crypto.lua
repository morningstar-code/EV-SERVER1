RegisterUICallback("ev-ui:getCrypto", function(data, cb)
...
RegisterUICallback("ev-ui:cryptoExchange", function(data, cb)
...
function processXCoinRedeem(cid)
  local hasItem = exports["ev-inventory"]:hasEnoughOfItem("inkedmoneybag", 1, false, true)
  if not hasItem then
    TriggerEvent("DoLongHudText", "You don't have what they're looking for", 2)
    return
  end
  local canRedeem = RPC.execute("ev-phone:xcoin:canRedeem", cid)
...
AddEventHandler("ev-phone:getXCoin", function()
...
RegisterUICallback("ev-ui:cryptoPurchase", function(data, cb)
...
RegisterUICallback("ev-ui:cryptoSell", function(data, cb)
  local character_id, bank_id, crypto_id, amount = data.character.id, data.character.bank_account_id, data.id, data.amount
...
AddEventHandler("ev-inventory:itemUsed", function(item)
  if not cryptoSticks[item] then return end

  local amount = cryptoSticks[item]

  local success, message = RPC.execute("phone:addCrypto", 2, amount)
  if success then
      TriggerEvent("chatMessage", "EMAIL ", 8, ("You have received %s crypto."):format(amount))
