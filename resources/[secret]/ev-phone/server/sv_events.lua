...
RPC.register("phone:getLicenses", function(pSource, pCharacterId)
    return getLicenses(pCharacterId)
end)
...
RPC.register("phone:addEmailContact", function(pSource, pData)
    return addEmailContact(pSource, pData.name, pData.email)
end)
...
RPC.register("ev:vehicles:getVehicleIds", function(pSource, pCharacterId)
    return getVehicleIds(pCharacterId)
end)

RPC.register("ev:vehicles:getRegistrationDocument", function(pSource, pDocumentId)
    return getRegistrationDocument(pDocumentId)
end)

RPC.register("ev:vehicles:getPlayerVehiclesWithCoordinates", function(pSource, pCharacterId)
...
AddEventHandler("phone:ping:request", function(pTargetId, pCoords, pIsAnon)
    local src = source
    return pingRequest(src, pTargetId, pCoords, pIsAnon)
end)

RegisterNetEvent("pingAccepted")
AddEventHandler("pingAccepted", function(pLastPingerId)
    local src = source
    return pingAccepted(src, pLastPingerId)
end)

RegisterNetEvent("phone:ping:rejected")
AddEventHandler("phone:ping:rejected", function(pLastPingerId)
    local src = source
    return pingRejected(src, pLastPingerId)
end)
