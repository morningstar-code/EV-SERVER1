...
AddEventHandler("FlipVehicle", function(pDummy, pEntity)
    ...
    local finished = exports["ev-taskbar"]:taskBar(30000, "Flipping Vehicle Over", true)
...
function GetVehicleAfterMarket(vehicle, type)
	if type == 'engineSound' then
		local data = RPC.execute("ev-vehicleSync:GetVehicleAfterMarket", vehicle, type)
		return data
	else
		local vehInfo = RPC.execute("ev-vehicleSync:GetVehicleAfterMarket", vehicle)
		return vehInfo
	end
end