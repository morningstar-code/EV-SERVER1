...
class Profiles extends React.Component<any> {
    ...
    refreshProfile = async (profile: any) => {
        const results = await Promise.all([
            mdtAction('getCivilianProfile', { profile: profile }, { returnData: profile }),
        ...
        const profileData = results[0].data; // getCivilianProfile
        const housingData = results[1].data; // ev-ui:getHousingInformation
        const hotelData = results[2].data; // ev-business:hotel:ui:getRooms
        const storageData = results[3].data; // ev-storageunits:client:getAccessLogs
        const vehicleData = results[4].data; // ev-mdt:getVehiclesByCharacterId
        const licenseData = results[5].data; // ev-ui:getLicenses
        const employmentData = results[6].data; // ev-ui:getEmploymentInformation
        const onlineStatus = results[7].data; // ev-mdt:getPlayerOnlineStatus
        const apartmentData = results[8].data; // ev-ui:getApartmentInformation
    ...
    suspendLicense = async (profile: any, license: any) => {
        ...
        await nuiAction('ev-ui:revokeLicense', { license: license, target_id: profile.id }) //This nui callback is inside np-gov/cl_state_management.lua
    ...
    addLicense = async (licenseId: number) => { //string
        showMdtLoadingModal(true);
        await nuiAction('ev-ui:grantLicense', { license: { id: licenseId }, target_id: this.props.profile.id }) //This nui callback is inside np-gov/cl_state_management.lua
