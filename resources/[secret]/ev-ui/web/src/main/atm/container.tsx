...
class Container extends React.Component<AtmState, { show: boolean }> {
    ...
    getAccounts = async () => {
        ...
        const defaultAccountIdx = accounts.findIndex((account) => account.bank_account_id === this.props.character.bank_account_id);
    ...
    clickGetTransactions = async (account: Account) => {
        ...
        const results = await this.getTransactions({ id: account.bank_account_id, access: account.access });
