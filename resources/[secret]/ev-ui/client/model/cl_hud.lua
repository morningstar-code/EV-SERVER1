...
local cmdsSet = {}
local vehBypass = {}
RegisterNUICallback("ev-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["ev-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["ev-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
...
AddEventHandler("ev-ui:watch", function()
...
AddEventHandler("ev:voice:proximity", function(proximity)
    sendAppEvent("hud", {
        voiceRange = proximity,
    })
end)

AddEventHandler("ev:voice:transmissionStarted", function(data)
    sendAppEvent("hud", {
...
AddEventHandler("ev:voice:transmissionFinished", function()
    sendAppEvent("hud", {
        voiceActive = false,
...
AddEventHandler('ev-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
local isAutopilotEnabled = false

AddEventHandler('ev-vehicles:autopilot:status', function (pStatus)
    isAutopilotEnabled = pStatus == 'started'
end)

AddEventHandler('ev-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('ev-weapons:client:setWeaponFireRate')
AddEventHandler('ev-weapons:client:setWeaponFireRate', function (pAmount)
...
RegisterNetEvent('ev-ui:hud:forceShowMinimap', function(pForce)
...
AddEventHandler('ev-drifting:showDriftMode', function (pToggle)
    showDriftMode = pToggle
    setHudValue("showDriftMode", pToggle)
end)

AddEventHandler('ev-drifting:toggleDriftMode', function (pToggle)
    driftMode = pToggle
...
AddEventHandler("ev-drifting:remainingTireThread", function (pThread)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["ev-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["ev-vehicles"]:CurrentFuel() or 0
end
...
function generateSpeedo()
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            setHudValue("autopilotShow", isAutopilotEnabled)
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            local degradation = exports["ev-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('ev-ui:setVehicleBypassed', function(veh, bypass)
...
AddEventHandler('ev-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["ev-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["ev-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("ev-island:onIsland", function(pState)
    onIsland = pState
end)
