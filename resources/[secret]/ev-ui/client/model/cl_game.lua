...
AddEventHandler('ev-config:configReady', function()
    loadPublicData()
end)

function loadPublicData()
    local showroomPurchase = exports['ev-config']:GetMiscConfig('showrooms.catalog.purchase')
    local skipMdwProfileCheck = exports['ev-config']:GetMiscConfig('mdw.profiles.skip')
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        local hasEmailAddress, emailAddress = RPC.execute("ev-phone:getEmailAddress")
        local character = {
            id = characterId,
...
AddEventHandler('ev-ui:updateCharacterData', function()
    sendCharacterData()
end)
...
AddEventHandler("ev-ui:restarted", function()
...
RegisterNetEvent("ev-weather:weatherHasChanged")
AddEventHandler("ev-weather:weatherHasChanged", function(pWeather)
