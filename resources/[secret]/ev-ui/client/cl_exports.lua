...
function RegisterUIEvent(eventName)
    if not Events[eventName] then
        ...
        RegisterNUICallback(eventName, function (...)
            TriggerEvent(('_cui_uiReq:%s'):format(eventName), ...)
        end)
    end
end
...
RegisterNetEvent("ev-ui:open-application")
AddEventHandler("ev-ui:open-application", openApplication)
...
function closeApplication(app, data)
  ...
  TriggerEvent("ev-ui:application-closed", app, { fromEscape = false })
end
