...
setInterval(() => {
  ...
  emitNet('ev-weather:client:weather', -1, currentWeather)
}, timePerWeather * 1000)

onNet('ev-weather:client:weather:request', () => {
  emitNet('ev-weather:client:weather', global.source, weatherProgression[0])
})

RegisterCommand('weather', (source: string, args: string[]) => {
  if (!IsPlayerAceAllowed(source, "ev-weather.commands.weather")) {
  ...
  emitNet('ev-weather:client:weather', -1, weatherProgression[0])
}, false)
