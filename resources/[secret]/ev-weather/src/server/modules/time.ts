...
setImmediate(() => {
  ...
  const curTime = Math.floor(time / 60);
  ...
  emit('ev-weather:server:nightTime', isNightTime);
  emitNet('ev-weather:client:time', -1, time);
})
...
RegisterCommand('time', (source: string, args: string[]) => {
  if (!IsPlayerAceAllowed(source, "ev-weather.commands.time")) {
...
onNet('ev-weather:client:time:request', () => {
  emitNet('ev-weather:client:time', global.source, time)
})
