...
setImmediate(() => {
  emitNet('ev-weather:client:weather:request')
})
...
const setWeather = async (weather: WeatherProgression, skipFreeze = false): Promise<void> => {
  ...
  emit('ev-weather:weatherHasChanged', currentWeather);
}

onNet('ev-weather:client:weather', async (weather: WeatherProgression) => {
  setWeather(weather)
})
