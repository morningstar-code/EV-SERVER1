...
export let UnitsConfig: UnitsConfig | null = null;
...
export async function InitConfig(): Promise<void> {
    while (UnitsConfig === null) {
        UnitsConfig = GetResourceConfig();
        await Delay(250);
    }
};

on("ev-config:configLoaded", async (pModule: string, pConfig: any) => {
    if (pModule === "main") {
